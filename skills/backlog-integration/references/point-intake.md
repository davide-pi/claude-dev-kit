...
## 1. Propose title and content, by role

The point is classified by **role**, not by a type name: the four roles are `user-story-standard`'s,
and which work item type each becomes on this project is resolved through `azdo-cli`. A role the
project has no type for is a question for the user, never the nearest-looking substitute.

The item content is **Italian**, and its shape belongs to `user-story-standard`: title, description
and acceptance criteria all follow that standard, for all four roles alike. Do not restate the
format here and do not invent a house variant. If that standard also produces an import artefact, it
is not used here — items are created straight on the board at step 5.

| Role | What to gather before proposing | Where it lands |
| --- | --- | --- |
| backlog item | the user need behind the point, plus what "done" means for it | description and acceptance criteria per `user-story-standard` |
| defect | three things, asked **explicitly and separately**, none of them mandatory — skip any the user cannot answer: how it is reproduced, what happens now, what should happen instead | the type's own fields, **never** `Description`, which the defect type may not have on its form — mapping in `azdo-cli` |
| technical activity | the technical work and why it is needed | free technical description, no acceptance criteria |
| analysis | what is blocked or unexplained, and by what | free technical description, no acceptance criteria |
...
| Field | Value |
| --- | --- |
| role | one of the four backlog-level roles |
| type | the type that role resolved to on this project, exactly as its process spells it |
| title | as confirmed |
| content | per role, in the fields that type actually has |
| assignee | a real identity, confirmed |
| parent | the focus feature |
| images | the confirmed file paths, or an explicit "no" |

**Done when:** the entry is in the cart with role, resolved type, title, content, assignee and the
image decision all confirmed by the user — proposed is not confirmed.
...
5. Acceptance criteria are written for a technical activity or an analysis item → the backlog-item
   shape was applied to every role → those two carry a technical description only.
