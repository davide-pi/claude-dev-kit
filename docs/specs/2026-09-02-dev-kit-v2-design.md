...
calls, then a **Never** line stating what the command must not do (post, force-push, drop, deploy).
...
| Validator | New rules: the line caps from section 2, every reference file reachable from its skill, no hardcoded version numbers in skill prose, and a Never line in every command |
