...
- **`-b [name]`** → **create a new** branch from the current `HEAD` and commit there. If `name` is
  omitted, generate a short kebab-case name from the commit summary — a branch name is an
  identifier and stays **English** even though the summary is Italian (e.g. `docs/wiki-split`).
...
6. **Report** — one block, in Italian: branch, short hash, files count, and the final message. If a hook failed,
   surface its output and stop — don't retry with hooks disabled.

## Message rules

- **Write the message in Italian**, subject and body alike. The code the commit describes — its
  identifiers and its comments — stays **English**: only the message is prose a person reads.
- One **concise imperative subject** (~50–72 chars): what changed, not how. An imperative subject in
  Italian is the second-person imperative (`Aggiungi`, `Correggi`, `Sposta`) or the infinitive
  (`Aggiungere`, `Correggere`) — whichever the repo already uses. Match the repo's existing commit
  style (skim `git log --oneline -10` if unsure); do not introduce a second convention.
- Reference the concrete area (module / file / feature) — no vague summaries. Identifiers, file
  paths, `AB#<id>` and `Fixes #<n>` are written as they are, never translated.
- No marketing words (seamlessly, robust, powerful, enhance — nor `migliora`, `ottimizza`,
  `potenzia`). No AI/tool footer.
